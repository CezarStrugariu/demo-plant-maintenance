...
sap.ui.getCore().attachInit(function (startParams) {
    function formatDate(date) {
        ...
        return formattedDate.replace(/(\d+)(\D)/, `$1${daySuffix} `);
    }

    function getDaySuffix(day) {
        const suffixes = ["th", "st", "nd", "rd"];
        const relevantDigits = day < 30 ? day % 20 : day % 30;
        const suffixIndex = relevantDigits > 3 ? 0 : relevantDigits;
    ...
    $.ajax({
        ...
        data: {
            ...
            rss_url: "https://www.realwire.com/rss/?id=308&row=&view=Synopsis",
        },
            ...
            data.items.forEach((item) => {
                ...
                if (!!item.pubDate) {
                    ...
                    let footer = new sap.m.Text("__nep" + ModelData.genID(), {
                        text: formatDate(new Date(item.pubDate)),
                    });
